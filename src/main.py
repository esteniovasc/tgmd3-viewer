...
def main():
    ...
    def open_editor_new(name, path):
        print(f"Criando projeto '{name}' em '{path}'")
        editor.setWindowTitle(f"{name} - TGMD-3 Viewer")
        home.hide()
        editor.showMaximized()

    # CORREÇÃO AQUI: A função agora aceita (path, data) vindos da Home
    def on_project_loaded(path, data):
        ...
        # Futuramente aqui passaremos os dados para o editor:
...
def main():
    ...
    # Home -> Abrir (CORREÇÃO DO NOME DO SINAL)
